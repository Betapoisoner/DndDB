...
export class BbegController {
    ...
    async update(request: Request, response: Response, next: NextFunction) {
        ...
        const bbegToUpdate = await this.userRepository.findOne({
            where: { id },
            relations: {
                actions: true,
                dungeons: true,
        ...
        const {
            // Destructure request body (more concise)
            name,
            photo,
            challenge_rating,
            experience_points,
        ...
        if (lair_actions !== undefined && Array.isArray(lair_actions) && lair_actions.length > 0)
            bbegToUpdate.lair_actions = lair_actions;
